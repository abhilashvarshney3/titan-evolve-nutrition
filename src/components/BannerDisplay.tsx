      ...
      {visibleBanners.map((banner) => (
        ...
        >
          ...
          <div className="container mx-auto relative z-10">
            <div className={`${position === 'hero' ? 'text-center' : position === 'top' ? 'flex flex-col md:flex-row items-center justify-between gap-4' : 'grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8 items-center'}`}>
              ...
              <div className={`${position === 'hero' ? 'space-y-4 md:space-y-8' : position === 'top' ? 'flex-1 space-y-1 md:space-y-2 text-center md:text-left' : 'order-1 lg:order-2 space-y-3 md:space-y-6'}`}>
                
                {/* Title with Animation */}
                <div className="relative">
                  {position === 'top' && banner.banner_type === 'announcement' ? (
                    <div className="overflow-hidden whitespace-nowrap">
                      <h3 className="text-base md:text-xl text-white drop-shadow-lg font-bold animate-[scroll_15s_linear_infinite] inline-block">
                        🔥 {banner.title} • Special Offer Available • Limited Time Only • 
                    ...
                    <h3 className={`font-black tracking-tight ${
                      position === 'hero' ? 'text-3xl md:text-5xl lg:text-7xl text-white leading-tight' : 
                      position === 'top' ? 'text-lg md:text-xl text-white drop-shadow-lg' : 'text-2xl md:text-4xl lg:text-5xl text-white leading-tight'
                    } animate-fade-in`}>
                      {banner.title}
                      {position !== 'top' && banner.banner_type === 'promotional' && (
                ...
                {/* Description */}
                {banner.description && position !== 'top' && (
                  <p className={`${
                ...
                {/* Enhanced CTA Button */}
                {banner.link_url && banner.button_text && position !== 'top' && (
                ...
                {banner.link_url && banner.button_text && position === 'top' && (
                  <div className="animate-fade-in" style={{ animationDelay: '0.6s' }}>
                    <Button
                      size="sm"
                      onClick={() => window.open(banner.link_url, '_blank')}
                      className="bg-white text-red-600 hover:bg-gray-100 font-bold shadow-lg hover:shadow-xl transition-all duration-300 px-4 md:px-6 py-2 text-sm md:text-base"
                    >
                      {banner.button_text}
                    </Button>
                  </div>
              ...
              {position === 'top' && banner.image_url && banner.banner_type !== 'announcement' && (
                <div className="flex-shrink-0 mt-2 md:mt-0 md:ml-4">
                  <img 
                    src={banner.image_url} 
                    alt={banner.title}
                    className="w-12 h-12 md:w-16 md:h-16 object-cover rounded-lg shadow-lg ring-2 ring-white/30 mx-auto md:mx-0"
                  />
                </div>
              )}
            </div>
